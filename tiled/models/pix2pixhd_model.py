...
class Pix2PixHDModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # self.loss_names = ['G_GAN', 'G_L1', 'G_Symmetry', 'D_real', 'D_fake']
        self.loss_names = ['G_GAN', 'G_Symmetry', 'D_real', 'D_fake']
        if self.isTrain:
            if opt.per_loss:
                self.loss_names.insert(2, 'G_Per')
            if opt.feature_matching_loss:
                self.loss_names.insert(3, 'G_FMatch')
    ...
    def set_input(self, input):
        ...
        # self.raw = input['C'].to(self.device)
    ...
    def backward_D(self):
        ...
        pred_fake = self.netD(fake_AB.detach())[-1]
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        # Real
        real_AB = torch.cat((self.real_A, self.real_B), 1)
        pred_real = self.netD(real_AB)[-1]
    ...
    def backward_G(self):
        ...
        self.loss_G_Symmetry = self.criterionSymmetry(self.fake_B)
        # combine loss and calculate gradients
        # self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_G_Symmetry
        self.loss_G = self.loss_G_GAN + self.loss_G_Symmetry
        ...
        if self.criterionFMatch is not None:
            self.loss_G_FMatch = self.criterionFMatch(pred_fake, pred_real) * 1000
